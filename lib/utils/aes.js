const { S_BOX, INVERSE_S_BOX, RCON, BLOCK_SIZE, KEY_SIZE } = require('./constants.js');
const { galoisMul2 } = require('./helpers.js');

function aesBlock(stateBytes, keyBytes, isDecrypt) {
  ...
  _aesBlock(state, key, isDecrypt);
  return state;
}

function _aesBlock(state, key, decrypt) {
  if (decrypt) {
