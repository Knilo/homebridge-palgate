...
class PalGatePlatform {
    ...
    try {
      ...
      let gates = data.devices.flatMap((deviceData) => {
        ...
        return gateEntries.map((gateEntry) => {
          ...
          if (custom) {
            ...
            // Expose virtual relay locks/switches if enabled and permitted by the API
            let exposeHoldOpenLock = false;
            let exposeHoldClosedLock = false;
            let exposeHoldOpenSwitch = false;
            let exposeHoldClosedSwitch = false;

            const isLatchPermitted = outputNum === 2 ? deviceData.output2Latch === true : deviceData.output1Latch === true;
            const hasRelayPermission = isLatchPermitted || deviceData.admin === true;

            if (hasRelayPermission) {
              let hasOverride = false;
              if (custom) {
                if (custom.relaySwitch === true || custom.relayLock === true) {
                  hasOverride = true;
                  exposeHoldOpenSwitch = custom.relaySwitch === true;
                  exposeHoldClosedSwitch = custom.relaySwitch === true;
                  exposeHoldOpenLock = custom.relayLock === true;
                  exposeHoldClosedLock = custom.relayLock === true;
                }
              }

              if (!hasOverride && this.config.enableRelayLocks === true) {
                if (this.config.relayAccessoryType && this.config.relayAccessoryType.toLowerCase() === 'switch') {
                  exposeHoldOpenSwitch = true;
                  exposeHoldClosedSwitch = true;
                } else {
      ...
      const customOnly = customGates
        ...
        .map(c => {
          ...
          if (c.relaySwitch === true || c.relayLock === true) {
            exposeHoldOpenSwitch = c.relaySwitch === true;
            exposeHoldClosedSwitch = c.relaySwitch === true;
            exposeHoldOpenLock = c.relayLock === true;
            exposeHoldClosedLock = c.relayLock === true;
  ...
  setupGarageDoorHandlers(accessory) {
    ...
    const customGates = this.config.customGates || [];
    const customEntry = customGates.find(item => item.deviceId === deviceId);

    // Retrieve customizable delays
    let openingDelay = this.config.gateOpeningDelay !== undefined ? parseInt(this.config.gateOpeningDelay, 10) : 1000;
    let gateCloseDelay = this.config.gateCloseDelay !== undefined ? parseInt(this.config.gateCloseDelay, 10) : 5000;

    if (customEntry) {
      if (customEntry.gateOpeningDelay !== undefined) {
        openingDelay = parseInt(customEntry.gateOpeningDelay, 10);
      }
      if (customEntry.gateCloseDelay !== undefined) {
        gateCloseDelay = parseInt(customEntry.gateCloseDelay, 10);
      }
    }
  ...
  setupLockHandlers(accessory) {
    ...
    // Lock remains unsecured for the cumulative duration of openingDelay + gateCloseDelay
    let openingDelay = this.config.gateOpeningDelay !== undefined ? parseInt(this.config.gateOpeningDelay, 10) : 1000;
    let gateCloseDelay = this.config.gateCloseDelay !== undefined ? parseInt(this.config.gateCloseDelay, 10) : 5000;
    ...
    if (accessory.context.lockCurrentState === undefined) {
      ...
      if (deviceData) {
        ...
        if (typeof deviceId === 'string' && deviceId.includes(':')) {
          outputNum = parseInt(deviceId.split(':').pop(), 10);
        }
  ...
  setRelayMode(accessory, mode, callback) {
    ...
    if (mode === 'hold_open') {
      ...
      latch = mode === 'hold_open';
      dsbl = mode === 'hold_open' || mode === 'hold_closed';
    }
