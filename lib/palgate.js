...
class PalGatePlatform {
  ...
  // in more than one account (a physically shared gate) is registered once — the first
  // account in config order owns it.
  async discoverDevices() {
    const { getDevices } = require('./api.js');

    // Admin status per base device — external-open detection reads the operation log,
    // which is admin-only (unlike relay mode, which needs latch permission). The poller uses
    // this to skip non-admin gates that would only ever 401. Populated across all accounts.
    ...
    for (const account of this.accounts) {
      ...
      this._buildDiscoveredGates(data.devices, customGates, account.id).forEach(gate => {
        if (seenDeviceIds.has(gate.deviceId)) {
          this.log.info(`Gate ${gate.deviceId} from account "${account.label}" is already managed by "${ownerLabel.get(gate.deviceId)}" — skipping duplicate.`);
          return;
        }
        seenDeviceIds.add(gate.deviceId);
        ownerLabel.set(gate.deviceId, account.label);
    ...
    const customOnlyGates = this._buildCustomOnlyGates(customGates, [...seenDeviceIds], this.accounts[0].id);
