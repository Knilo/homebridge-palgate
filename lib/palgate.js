        ...
        if (value === this.Characteristic.TargetDoorState.OPEN) {
          ...
          this.openGateForAccessory(accessory, (err) => {
            ...
            setTimeout(() => {
              ...
              // Phase 3: Transition to CLOSING after openDelay
              setTimeout(() => {
                accessory.context.currentDoorState = this.Characteristic.CurrentDoorState.CLOSING;
                accessory.context.targetDoorState = this.Characteristic.TargetDoorState.CLOSED;
                service.setCharacteristic(this.Characteristic.CurrentDoorState, this.Characteristic.CurrentDoorState.CLOSING);
                service.setCharacteristic(this.Characteristic.TargetDoorState, this.Characteristic.TargetDoorState.CLOSED);
                this.log.info("Garage door starting to close for", accessory.displayName);

                // Phase 4: Transition to CLOSED after closingDelay
                setTimeout(() => {
                  accessory.context.currentDoorState = this.Characteristic.CurrentDoorState.CLOSED;
