...
class PalGatePlatform {
  constructor(log, config, api) {
    ...
    try {
      ...
      this.log.error("Failed to generate or validate token:", err.message);
      return;
    }
  ...
  async discoverDevices() {
    ...
    try {
      ...
      this.log.error("Failed to generate temporal token:", err.message);
      return;
    }
  ...
  openGateForAccessory(accessory, callback) {
    ...
    try {
      ...
      this.log.error("Failed to generate token or open gate for", accessory.displayName, ":", err.message);
      callback(err);
    }
  }
}
