...
class PalGatePlatform {
  ...
  async discoverDevices() {
    ...
    // Track admin status per base device — external-open detection reads the operation log,
    // which is admin-only (unlike relay mode, which needs latch permission). The poller uses
    // this to skip non-admin gates that would only ever 401.
    this._deviceAdminById = new Map();
    data.devices.forEach((deviceData) => {
      const id = deviceData.id || deviceData._id;
      if (id) this._deviceAdminById.set(String(id), deviceData.admin === true);
    });
    ...
    return devices.flatMap((deviceData) => {
      ...
      return generateGateEntries(deviceId, outputs, defaultName, deviceData).map((gateEntry) => {
        ...
        if (custom) {
          ...
          return { deviceId: gateDeviceId, name, exposeGarageDoor, exposeSwitch, exposeLock, exposeHoldOpenLock, exposeHoldClosedLock, exposeHoldOpenSwitch, exposeHoldClosedSwitch, exposeHoldOpenValve, exposeHoldClosedValve, deviceData };
        } else {
          ...
          return { deviceId: gateDeviceId, name: gateEntry.name, exposeGarageDoor, exposeSwitch, exposeLock, exposeHoldOpenLock, exposeHoldClosedLock, exposeHoldOpenSwitch, exposeHoldClosedSwitch, exposeHoldOpenValve, exposeHoldClosedValve, deviceData };
        }
      });
  ...
  _buildCustomOnlyGates(customGates, discoveredIds) {
    return customGates
      ...
      .map(c => {
        ...
        return { deviceId: c.deviceId, name, exposeGarageDoor, exposeSwitch, exposeLock, exposeHoldOpenLock, exposeHoldClosedLock, exposeHoldOpenSwitch, exposeHoldClosedSwitch, exposeHoldOpenValve, exposeHoldClosedValve };
      });
  }
  ...
  async _pollExternalOpens() {
    ...
    try {
      ...
      this.log.warn("Log poller: failed to generate token:", err.message);
      return;
    }
    ...
    await Promise.all(baseIds.map(async baseId => {
      try {
        ...
        this._processLogEntries(baseId, entries);
      } catch (err) {
        anyFailure = true;
  ...
  _processLogEntries(baseId, entries) {
    ...
    for (const entry of fresh) {
      ...
      // Self-dedupe: skip our own HomeKit-initiated opens (our phone number within ±30s of a
      // trigger the plugin just issued). Opens by our phone number OUTSIDE that window are the
      // owner opening from the PalGate app well after any HomeKit action — those are genuine
      // "not from HomeKit just now" opens and SHOULD be surfaced.
      const entryUserMatchesUs = this.phoneNumber !== undefined && String(entry.userId) === String(this.phoneNumber);
