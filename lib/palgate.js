...
class PalGatePlatform {
      ...
      let gates = data.devices.flatMap((deviceData) => {
        ...
        return gateEntries.map((gateEntry) => {
          ...
          if (custom) {
            ...
            let gateEnableRelayLocks = this.config.enableRelayLocks;
            let gateRelayAccessoryType = this.config.relayAccessoryType;
            if (custom) {
              if (custom.enableRelayLocks === 'enabled') {
                gateEnableRelayLocks = true;
              } else if (custom.enableRelayLocks === 'disabled') {
                gateEnableRelayLocks = false;
              }
              if (custom.relayAccessoryType !== undefined && custom.relayAccessoryType !== '') {
                gateRelayAccessoryType = custom.relayAccessoryType;
              }
            }

            if (gateEnableRelayLocks === true) {
              ...
              if (isLatchPermitted || deviceData.admin === true) {
                if (gateRelayAccessoryType === 'switch') {
            ...
            if (this.config.enableRelayLocks === true) {
              const isLatchPermitted = outputNum === 2 ? deviceData.output2Latch === true : deviceData.output1Latch === true;
              if (isLatchPermitted || deviceData.admin === true) {
                if (this.config.relayAccessoryType === 'switch') {
                  exposeHoldOpenSwitch = true;
                  exposeHoldClosedSwitch = true;
                } else {
                  exposeHoldOpenLock = true;
  ...
  setupGarageDoorHandlers(accessory) {
    ...
    let openingDelay = this.config.gateOpeningDelay !== undefined ? parseInt(this.config.gateOpeningDelay, 10) : 0;
    let openDelay = this.config.gateOpenDelay !== undefined ? parseInt(this.config.gateOpenDelay, 10) : 5000;

    if (customEntry) {
      if (customEntry.gateOpeningDelay !== undefined) {
        openingDelay = parseInt(customEntry.gateOpeningDelay, 10);
      }
      if (customEntry.gateOpenDelay !== undefined) {
        openDelay = parseInt(customEntry.gateOpenDelay, 10);
      }
    }
