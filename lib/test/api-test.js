...
const phoneNumber = '';
const tokenType = 1;
const openByValue = process.argv[3]; // e.g. node api-test.js new 100
...
if (apiMode === 'old') {
  ...
  (async () => {
    ...
    if (deviceId) {
      ...
      const openByLabel = openByValue !== undefined ? ` (openBy=${openByValue})` : ' (no openBy)';
      await measureApiCall(`openGate${openByLabel}`, async () => await openGate(deviceId, token, openByValue !== undefined ? openByValue : undefined));
    } else {
      console.log('Device ID not provided. Skipping getDeviceInfo and openGate tests.');
    }
