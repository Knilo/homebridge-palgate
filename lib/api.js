...
// The user/log endpoint wraps its payload ({ log, err, msg, status }). Return the
// log array directly so callers don't read entries off the wrapper (same envelope
// class of bug as device/{id}/ — see unwrapDevice). Returns [] when the array is
// missing so pollers never iterate undefined.
function unwrapLog(response) {
    if (response && typeof response === 'object' && Array.isArray(response.log)) return response.log;
    if (Array.isArray(response)) return response;
    return [];
}

// Retrieves the recent operation log for a device (last ~50 operations, newest first).
async function getDeviceLog(tokenHeader, deviceId) {
    const endpoint = `user/log?id=${encodeURIComponent(deviceId)}`;
    return unwrapLog(await callApi(endpoint, tokenHeader));
}

// Single-shot variant for use in the external-open poller.
async function getDeviceLogOnce(tokenHeader, deviceId) {
    const endpoint = `user/log?id=${encodeURIComponent(deviceId)}`;
    return unwrapLog(await callApiOnce(endpoint, tokenHeader));
}
